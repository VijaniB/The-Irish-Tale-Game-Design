...
function newGame() {
    let gameOver = false,
        ...
        myMusic = new sound("assets/theme.mp3");
         myMusic.loop(); myMusic.play();
    const groundPosY = canvas.height - groundHeight;

    let man = new Man(100, groundPosY),
        sheeps = [],
        lastSheepAdded = Date.now() + 5000;  // give some time before the first enemy to appear
        difficulty = 0;

    function restartGame() {
        sheeps = [];
        man.restart();
        lastSheepAdded = Date.now();
        gameOver = false;
        gameWon = false;
        gameRestarting = false;
        difficulty = 0;
    ...
    (function update() {
        if (!gamePaused) {
            if (gameRestarting) {
                ...
                sheeps.forEach(s => {
                    s.update();
                    if (checkCollision(man, s)) {
                        gameOver = true;
                        s.attack();
                    }
                });

                sheeps = sheeps.filter(s => s.pos_x > -100);
                if (Date.now() - lastSheepAdded > 1700) {
                    if (Math.random() > 0.5) {
                        sheeps.push(new Sheep(canvas.width, groundPosY));
                    }
                    lastSheepAdded = Date.now();
                }
